...
6. Use `web_fetch` when a search snippet is insufficient to establish the period, unit, scope, attribution, or exact wording of a material claim. For a delegated research lane, combine related topics into broad searches, fetch only the most material primary-source URLs, and accept partial evidence with explicit limitations when its bounded search/fetch allowance is consumed.
