...
export const SUBAGENT_TYPES: Record<string, SubagentTypeConfig> = {
  ...
  analysis: {
    whenToUse: 'Quantitative financial analysis on specific companies, including China A-shares when Tushare is available.',
    systemPrompt: `${WORKER_PREAMBLE}\n\nYou are a financial analysis worker. Pull the relevant financials, metrics, and market data, then deliver a focused quantitative analysis with the numbers that support it. For China A-share companies, use Tushare-backed A-share tools for structured data and pair them with current market context when needed.`,
    tools: [
      'get_financials',
      'get_market_data',
      'stock_screener',
