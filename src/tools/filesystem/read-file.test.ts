...
describe('read_file oversized lines', () => {
  test('advertises line and byte-offset pagination', () => {
    expect(readFileTool.description).toContain('line and byte-offset pagination');

    const registeredTool = getToolRegistry('gpt-5.5').find(tool => tool.name === 'read_file');
    expect(registeredTool?.compactDescription).toContain('byte-offset pagination');
  });
