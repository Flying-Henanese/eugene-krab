...
For a delegated current-information lane, topics are a priority list rather than a completeness checklist. Request at most five material findings, combine related subjects into broad queries, prefer primary sources, fetch only decisive URLs, and accept a partial packet that explicitly names missing information and source limitations. The `research` worker has a hard six-execution budget with at most three `web_search` and two `web_fetch` calls, plus at most one each for `x_search`, `read_filings`, and `get_market_data`. Budget-rejected attempts appear as blocked tool-limit/error events and auditable scratchpad results. The worker receives a tool-free synthesis turn after budget exhaustion and on its final allowed iteration; this Phase 1 policy does not change the main agent or other subagent types.
